...
## Face-track temporal consistency (2026-09-23, face_track.py)

New measurement layer: consecutive-frame face-crop embedding drift
(SBI EfficientNet-B0 backbone features), normalized landmark jitter,
and face-box area smoothness. Wired into the video deep-layer as
"얼굴 트랙 시간-일관성" (cap 35).

Corpus: 2 real talking-head clips (PLOS ONE / WIKITONGUES, Commons) vs
synthetic swaps generated by `scripts/gen_faceswap_video.py`.

| Video | score | drift mean | jitter | areaΔ |
|---|---:|---:|---:|---:|
| talk_plos (real) | 0 | 0.032 | 0.010 | 0.014 |
| talk_wikitongues (real) | 0 | 0.019 | 0.020 | 0.035 |
| **swap_jitter** (per-frame random source) | **90** | 0.323 | 0.177 | 0.925 |
| swap_smooth (single source, box-aligned) | 0 | — | — | 0 faces |

- Jitter-mode swaps separate by ~10x on every metric — the heuristic
  score fires correctly.
- **Evasion found**: a smooth single-identity paste defeats re-detection
  (mediapipe finds no face in the composited frames) → unavailable, not
  "clean". Landmark-aligned deepfakes will similarly evade.
- Thresholds are calibrated on 3 videos — advisory only, per module
  docstring.

## SyncNet learned lip-sync — verified working (2026-09-23)

`syncnet-python 0.2.2` + `models/syncnet_v2.model` + `sfd_face.pth`
run end-to-end on CPU:

| Video | offset | confidence | score |
|---|---:|---:|---:|
| talk_plos aligned | +0f | 5.1 | 0 |
| talk_plos +400ms shifted | **-10f = exactly -0.4s** | 4.2 | 40 |
| talk_wikitongues aligned | — | S3FD no track | → heuristic r=0.19, score 0 |
| talk_wikitongues +400ms | — | S3FD no track | → heuristic lag+1.94s, score 25 |

- SyncNet recovered the injected offset exactly; the learned path is live.
- S3FD misses some real faces → now falls back to the envelope heuristic
  instead of returning unavailable (fixed 2026-09-23).
