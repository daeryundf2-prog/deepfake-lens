"""Rule-based classifier for AI-generated image screening.

Scores images with fixed feature thresholds and, when feature statistics
from labelled data are provided, z-score deviations. This is not a trained
ML model — the "training" entry point only computes per-feature mean/std
statistics — so scores are prioritization evidence, not learned
probabilities.
"""
...
class RuleClassifier:
    """Rule-based classifier over extracted image features.

    Without feature statistics it applies fixed threshold rules; with
    statistics from ``train_rule_classifier`` it scores z-score deviations
    of the same features. Neither path is a learned model.
    """
    ...
    def predict(self, features: dict[str, float]) -> ClassificationResult:
        ...
        # Use statistical comparison if we have feature statistics
...
def train_rule_classifier(training_data: list[dict[str, Any]]) -> RuleClassifier:
    """Compute per-feature mean/std statistics from labelled data.

    Despite the historical name this does not fit a model — it only
    populates the feature statistics used by the z-score rules.
    """
...
def save_classifier(classifier: RuleClassifier, path: Path) -> None:
...
def load_classifier(path: Path) -> RuleClassifier:
...
# Deprecated names kept for backward compatibility; new code should use
# RuleClassifier / train_rule_classifier.
SimpleClassifier = RuleClassifier
train_simple_classifier = train_rule_classifier
