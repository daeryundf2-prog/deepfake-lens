...
- `path`, `name`, `kind` (`image`, `text`, `unsupported`, `duplicate`,
  `unknown`), `status` (`analyzed`, `failed`, `skipped`, `unsupported`,
  `duplicate`), `size_bytes`, `error`, `duplicate_of`.
...
- `model_analysis`: `null`, or `{available, ...}` describing the external
  model sidecar result. `summary.external_model_active` counts items where
