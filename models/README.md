...
Runtime profiles for published AI-generated-image detectors. Profiles are
committed; **weights never are** (license + size — `models/*.pth` and friends
are gitignored). A scan treats every profile as optional: missing checkpoints
or missing optional dependencies degrade to `available: false` entries with
the fetch hint in `limitations`, never a crash.
...
Default scans still use the single verified engine (`aide-runtime.json`); the
zoo is opt-in because the placeholder members add latency without scores
until their checkpoints are fetched.
